...
export async function PATCH(
  ...
  try {
    ...
    if (error) {
      console.error('Error updating admin forum reply:', error);
      return NextResponse.json({ error: 'Failed to update reply' }, { status: 500 });
    }

    return NextResponse.json({ data });
  } catch (error) {
    console.error('Error in PATCH /api/admin/forum/replies/[id]:', error);
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      console.error('Error deleting admin forum reply:', error);
      return NextResponse.json({ error: 'Failed to delete reply' }, { status: 500 });
    }

    return NextResponse.json({ message: 'Reply deleted successfully' });
  } catch (error) {
    console.error('Error in DELETE /api/admin/forum/replies/[id]:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
